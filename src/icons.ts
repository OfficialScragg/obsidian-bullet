...
const PATHS: Record<string, string> = {
	...
	star: '<path d="M12 2.6l2.95 5.98 6.6.96-4.77 4.65 1.13 6.57L12 17.66l-5.91 3.1 1.13-6.57L2.45 9.54l6.6-.96z"/>',
...
export function bulletIcon(el: HTMLElement, name: string): HTMLElement {
	...
	svg.addClass("bl-svg");
	// Tolerate a bare path string as well as full markup. Setting raw path
	// data as innerHTML yields text and draws nothing at all, which is exactly
	// how the priority star came to be invisible.
	svg.innerHTML = path.trimStart().startsWith("<")
		? path
		: `<path d="${path}"/>`;
	el.appendChild(svg);
	return el;
...
/** Icon names Bullet knows about, for the self-check in tests. */
